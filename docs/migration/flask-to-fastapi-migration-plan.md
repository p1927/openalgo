...
> **Counts are a stale snapshot, not live config.** The broker/importer counts quoted below (`master_contract_db.py` modules, `extensions.socketio` importers, etc.) were captured at different points on two independently-edited copies of this document and were merged without reconciling them — some sections still say 33/43-44, others say 36/48. Neither is accurate against the current tree (as of this audit: 37 `master_contract_db.py` files, ~54 `extensions.socketio` importers) — new brokers and modules have been added since either snapshot was taken. Re-derive these counts from the current tree before using them to scope real work; do not treat any specific number in this document as current.
