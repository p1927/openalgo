...
# Load our custom tool additions from a sidecar file rather than editing
# them into this file directly, so re-syncing with upstream never has to
# reconcile our tool additions against upstream's own edits here. This
# can't be a normal `from mcp import custom_tools` import: `mcp/` is not
# a package (no __init__.py) and the name collides with the pip-installed
# `mcp` SDK (see utils/mcp_tool_registry.py:_load_mcpserver_module for the
# same constraint). Must run before _finalize_registry() below so the
# custom tools' @mcp.tool() registrations are included in it.
_custom_tools_path = os.path.join(os.path.dirname(__file__), "custom_tools.py")
if os.path.exists(_custom_tools_path):
    import importlib.util as _ilu

    _custom_tools_spec = _ilu.spec_from_file_location(
        "openalgo_mcp_custom_tools", _custom_tools_path
    )
    _custom_tools_module = _ilu.module_from_spec(_custom_tools_spec)
    _custom_tools_spec.loader.exec_module(_custom_tools_module)
